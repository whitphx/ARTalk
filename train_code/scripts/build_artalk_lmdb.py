...
Input: JSONL manifests with one clip per row carrying ``sample_id``,
``dataset``, ``audio_path`` (any rate and channel layout; resampled to
16 kHz mono here) and ``gt_smoothed_pkl_path`` (a ``smoothed.pkl`` of
per-frame FLAME codes). Output, under ``--out``:
...
the released checkpoints (the training configs' 108-dim layout belongs to
a different data revision).
...
# frames (the style window needs 100 frames); this cut is one frame
# stricter, so nothing stored is dead weight for any CLIP_LENGTH <= 100.
MIN_FRAMES = 102
...
def main() -> None:
    ...
    for i, r in enumerate(items):
        ...
        if motion.shape[0] <= MIN_FRAMES:
            skipped["too short"] += 1
            continue
        # Reject pairs whose audio and motion tracks disagree by more than
        # five frames; the loader tolerates smaller drift.
    ...
    if stats_count == 0:
        raise RuntimeError("no clip landed in the train split; stats would be empty")
