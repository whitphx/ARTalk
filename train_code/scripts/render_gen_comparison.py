...
def main() -> None:
    ...
    def render_track(motion):
        ...
        for s in range(0, motion.shape[0], 50):
            ...
            verts = model.base_codec.face_decoder.get_flame_verts(
                batch[None], with_headpose=False
            )[0] if hasattr(model.base_codec, "face_decoder") else None
            if verts is None:
                from core.libs.flame_model import FLAMEModel
                if not hasattr(render_track, "_flame"):
                    render_track._flame = FLAMEModel(n_shape=300, n_exp=100).eval().to(args.device)
                verts = render_track._flame.get_flame_verts(batch[None], with_headpose=False)[0]
            rgb = mesh(verts)[0]  # (T, 3, H, W) in 0..255
            frames.append(rgb.to(torch.uint8).cpu())
        return torch.cat(frames, dim=0)

    meta = json.load(open(args.data / "metadata.json"))["test"]
